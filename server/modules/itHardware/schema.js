...
const seedAssets = [
  ...
  ["AST-LAP-002", "QPMS-LAP-002", "Laptop", "HP", "EliteBook 840", "HP840QPMS002", "Available", "", "2027-04-20", "2024-04-20", "IT Store"],
  ["AST-MON-004", "QPMS-MON-004", "Monitor", "LG", "24MP60G", "LG24QPMS004", "Available", "", "2026-12-01", "2023-12-01", "IT Store"],
...
export function ensureItHardwareSchema(pool) {
  if (!schemaReadyPromise) {
    schemaReadyPromise = prepareSchema(pool).catch((error) => {
...
async function prepareSchema(pool) {
  await pool.query(`
    ...
    create sequence if not exists ${SCHEMA}.it_hardware_ticket_number_seq;

    create table if not exists ${SCHEMA}.it_employee (
    ...
    create table if not exists ${SCHEMA}.it_asset (
    ...
    create table if not exists ${SCHEMA}.it_hardware_ticket (
      ...
      employee_id text references ${SCHEMA}.it_employee(employee_id),
      employee_name text not null,
      employee_email citext not null,
      department text,
      designation text,
      hardware_category text not null,
      asset_id text references ${SCHEMA}.it_asset(asset_id),
    ...
    create index if not exists idx_it_hardware_ticket_employee on ${SCHEMA}.it_hardware_ticket (employee_email);
    create index if not exists idx_it_hardware_ticket_status on ${SCHEMA}.it_hardware_ticket (ticket_status);
    create index if not exists idx_it_hardware_ticket_type on ${SCHEMA}.it_hardware_ticket (ticket_type);

    create table if not exists ${SCHEMA}.it_ticket_audit_log (
    ...
    create table if not exists ${SCHEMA}.it_email_notification (
    ...
    create table if not exists ${SCHEMA}.it_employee_import_batch (
  ...
  await seedTicketIfMissing(pool);
}

async function seedTicketIfMissing(pool) {
  const existing = await pool.query(`select 1 from ${SCHEMA}.it_hardware_ticket limit 1`);
